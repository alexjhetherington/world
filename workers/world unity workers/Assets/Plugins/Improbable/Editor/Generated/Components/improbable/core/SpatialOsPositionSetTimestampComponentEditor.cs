...
{
    [CustomEditor(typeof(SpatialOsPositionSetTimestampComponent))]
    public class SpatialOsPositionSetTimestampComponentEditor : UnityEditor.Editor
    {
        private SpatialOsPositionSetTimestampComponent editedComponent;
        private SpatialOsPositionSetTimestampComponentEditorData componentData;

        private class SpatialOsPositionSetTimestampComponentEditorData : SpatialOsComponentEditorBase<SpatialOsPositionSetTimestampComponent>
        {
            public float Timestamp
            {
                get { return Component.Timestamp; }
            }
        }

        protected virtual void OnEnable()
        {
            editedComponent = (SpatialOsPositionSetTimestampComponent) target;

            var hashCode = editedComponent.GetHashCode();
            componentData = (SpatialOsPositionSetTimestampComponentEditorData) EditorObjectStateManager.GetComponentEditorData(hashCode);
            if(componentData == null)
            {
                componentData = new SpatialOsPositionSetTimestampComponentEditorData();
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.Space();
            EditorGUILayout.FloatField("timestamp", componentData.Timestamp);
