...
{
    [CustomEditor(typeof(SpatialOsClientAuthorityCheckComponent))]
    public class SpatialOsClientAuthorityCheckComponentEditor : UnityEditor.Editor
    {
        private SpatialOsClientAuthorityCheckComponent editedComponent;
        private SpatialOsClientAuthorityCheckComponentEditorData componentData;

        private class SpatialOsClientAuthorityCheckComponentEditorData : SpatialOsComponentEditorBase<SpatialOsClientAuthorityCheckComponent>
        {        }

        protected virtual void OnEnable()
        {
            editedComponent = (SpatialOsClientAuthorityCheckComponent) target;

            var hashCode = editedComponent.GetHashCode();
            componentData = (SpatialOsClientAuthorityCheckComponentEditorData) EditorObjectStateManager.GetComponentEditorData(hashCode);
            if(componentData == null)
            {
                componentData = new SpatialOsClientAuthorityCheckComponentEditorData();
